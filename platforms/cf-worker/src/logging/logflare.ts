				...
				if (Array.isArray(i.messages) && i.messages.length > 0) {
					firstMessage =
						i.messages[0] && typeof i.messages[0] === 'object' && 'toString' in i.messages[0]
							...
							: '' + i.messages[0];
					moreMessages = i.messages.slice(1).map((m) => (m && typeof m === 'object' && 'toString' in m ? m.toString() : '' + m));
				}
				if (i.extra && Array.isArray(i.extra)) {
					moreMessages.push(...i.extra.map((m) => (m && typeof m === 'object' && 'toString' in m ? m.toString() : '' + m)));
				}
				return {
					level: i.level,
